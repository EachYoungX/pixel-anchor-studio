...
describe('desktop drop authorization ordering', () => {
  ...
  it('keeps native drag feedback separate from Rust-authorized file reading', () => {
    ...
    expect(nativeCallback).toContain('const type = event.payload.type')
    expect(nativeCallback).toContain('desktopDrop.handleNativeDrag(type)')
    expect(nativeCallback).not.toContain('handleIncomingFiles')
    expect(nativeCallback).not.toContain('read_dropped_file')
    expect(nativeCallback).toContain('claimAuthorizedDrop()')
    expect(composable).toContain("invoke<DesktopDroppedPayload | null>('claim_authorized_drop')")
